...
function CourseDetail() {
  ...
  useEffect(() => {
    ...
    try {
      axios
        .get("http://localhost:8000/course/" + course_id + "/")
        .then((res) => {
    ...
    try {
      axios
        .get(
          "http://localhost:8000/fetch-enroll-status/" +
            loggedstudentId +
            "/" +
            course_id +
            "/"
  ...
  try {
    axios
      .get(
        "http://localhost:8000/fetch-rating-status/" +
          loggedstudentId +
          "/" +
          course_id +
          "/"
  ...
  const enrollCourse = () => {
    ...
    try {
      axios
        .post("http://localhost:8000/student-enroll-course/", _formData)
        .then((res) => {
          console.log(res.data);
  ...
  const markasFavourite = () => {
    ...
    try {
      axios
        .post(
          "http://localhost:8000/student-add-favourite-course/",
          _formData,
          {
            headers: {
  ...
  const removeFavourite = (pk) => {
    ...
    try {
      axios
        .get(
          "http://localhost:8000/student-remove-favourite-course/" +
            course_id +
            "/" +
            loggedstudentId +
            "/",
  ...
  const formSubmit = () => {
    ...
    try {
      axios
        .post("http://127.0.0.1:8000/course-rating/", _formData)
        .then((res) => {
          window.location.reload();
        });
    } catch (error) {
    ...
    <div className="container mt-3">
      <div className="row">
        ...
        <div className="col-8">
          ...
          <p className="fw-bold">
            Course By:{" "}
            <Link to={`/teacher-detail/${teacherData.teacherId}`}>
          ...
          <p className="fw-bold">
            Students Enrolled: {courseData.total_enrolled_students} students
          </p>
          ...
          {userLoginStatus === "success" && favouriteStatus === "success" && (
            <p>
              <button
                ...
                title="Remove your favourite course list"
                type="button"
          ...
          {chapterData.map((chapter, index) => (
            <ul key={index} className="list-group list-group-flush">
              <li className="list-group-item">
                ...
                >
                  <i className="bi bi-play" onClick={handleShow}></i>
                </button>{" "}
                <button
                  ...
                  data-bs-target="#videoModal1"
                >
                  <i className="bi bi-stop" onClick={handleClose}></i>
                </button>{" "}
              </li>
              ...
              {isOpen && (
                ...
                >
                  <video controls width="600" style={{ margin: "2rem 0" }}>
                    <source src={chapter.video} type="video/webm" />
                    <source src={chapter.video} type="video/mp4" />
                  </video>
                  <p style={{ color: "grey" }}>{chapter.description}</p>
                </div>
              )}
          ...
          <div className="col-md-3 mb-4" key={index}>
            ...
            >
              <Link target="__self" to={`/detail/${rcourse.pk}`}>
                <img
                  src={`${siteUrl}media/${rcourse.fields.thumbnail}`}
